...
function Doctordash() {
  ...
  const sendMeetLinkToPatient = async () => {
    if (!selectedAppointment) {
      toast.error("No appointment selected");
      return;
    }
    ...
    try {
      ...
      const response = await fetch('http://localhost:4000/api/email/send-meet-link', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          userId: selectedAppointment.bookedPatient._id,
          meetLink: meetLink,
          appointmentTime: `${selectedAppointment.appointmentDate} ${selectedAppointment.appointmentTime}`,
      ...
      if (!response.ok) {
        throw new Error(data.message || 'Failed to send meet link');
      }
  
      toast.success(data.message || 'Meet link sent successfully!');
      setMeetLink('');
      
    } catch (error) {
      console.error('Error sending meet link:', error);
      toast.error(error.message || 'Failed to send meet link');
    } finally {
                      ...
                      {selectedAppointment.appointmentType === 'Online Consultation' && (
                        <div>
                          ...
                          <div className="flex gap-2">
                            ...
                            <button
                              className={`flex items-center gap-1 px-4 py-2 rounded-md transition-colors ${
                                !meetLink || isSendingLink
                                  ? 'bg-gray-300 text-gray-600 cursor-not-allowed'
                                  : 'bg-blue-500 text-white hover:bg-blue-600'
                              }`}
                              onClick={sendMeetLinkToPatient}
                              disabled={!meetLink || isSendingLink}
                            >
                              {isSendingLink ? (
                                <>
                          ...
                          <p className="text-xs text-gray-500 mt-1">
                            This will send the link to {selectedAppointment.bookedPatient.name}'s email
                          </p>
                        </div>
                      )}
